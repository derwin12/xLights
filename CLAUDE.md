...
- **`std::isnan(x)` / `std::isinf(x)` / `std::isfinite(x)` as defensive guards** — these may be folded to constants (`false`/`false`/`true`) in Release, making the guard a no-op. **Use `__builtin_isnan(x)` / `__builtin_isinf(x)` / `__builtin_isfinite(x)`** — clang preserves these even under `-ffinite-math-only`.
